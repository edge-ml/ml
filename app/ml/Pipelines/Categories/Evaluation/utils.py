...
def calculateMetrics(y_test, y_pred, target_labels):
    ...
    performance_results['confusion_matrix'] = json.dumps(confusion_matrix(y_test, y_pred, labels=num_labels), cls=JSONEncoder)
    performance_results['classification_report'] = classification_report(y_test, y_pred, labels=num_labels, zero_division=0, target_names=target_labels, output_dict=True)
